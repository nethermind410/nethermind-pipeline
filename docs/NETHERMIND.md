...
**AI engines (Control → AI engines, `llm.py`).** Every piece of writing — research, scripts, long-form, packaging, comment replies — goes through one layer that picks the engine per job: your Claude subscription, an Anthropic API key (pay per use, with web search), any OpenAI-compatible service (OpenAI, OpenRouter, Gemini), or a free local model through Ollama. Each job has a chain; an answer that errors, refuses, hits a limit or fails the job's checks goes to the next engine, so a cheap engine can cost a retry, never a broken video. Research only goes to engines that can search the web. Presets: *Claude only* (the default — nothing changes), *Never stop* (API key carries on when the subscription limit hits), *Lean* (Claude researches and writes; cheaper engines package and reply, Claude as the safety net). Keys live only in `.env`; every call is logged with its cost in `out/llm_usage.jsonl`. vidIQ title scoring stays on Claude (it uses vidIQ's Claude connector).
